...
Hamming = (function() {
    ...
    //              of parity bits, and the total length of the enocoded data.
    //
    //  OUTPUTS : none
    //
    //  RETURNS : none
    ...
    //              also higlights a given syndrome that corresponds to an
                            ...
                            errorSyndrome)
    {
        ...
        //*************************************************************
        //  Determine which column in the syndrome or parity matricies
        ...
        for (row = numberOfSyndromeRows-1; row >= 0; row--)
        {
            ...
            for (column = numberOfParityBits - 1; column >= 0 ; column--)
            {
                ...
                {
                    //  higlight the entry if it is part of the error syndrome
        ...
        if (allowModification == false)
        {
            ...
            else
            {
                if (useExtraParityBit == true)
                {
                    //  if the error syndrome is not 0, but it is not found in the syndome
                    //  or parity matricies, assume a double bit error
    ...
    */
    ...
    //          calculating and error syndrom based on a syndrome
    ...
    //  INPUTS : NONE
    //
    //  DESCRIPTION : Deletes the syndromMatrix and ParityMatrix so that the will
    //                  be recalculated based on the cahnged data
    //
    //  OUTPUTS : none
    //
    //  RETURNS : none.
    ...
    //  INPUTS : none
    //
    //  DESCRIPTION : Clears the inoput form
    //
    //  OUTPUTS : none
    //
    //  RETURNS : none.
    ...
    function computeHammingCode()
    {
        ...
        if (checkInputs())
        {
            //  convert input data to bianry array for parity bits
            //  calculation
            if (document.inputForm.Base[0].checked)  //  binary
            {
                inputDataLength = document.inputForm.inputDataTextbox.value.length;
            ...
            else  // hex
            {
                ...
                returnValue = stringToBinaryArray(16,
                                        document.inputForm.inputDataTextbox.value,
                                        inputDataArray,
                                        inputDataLength);
            }
            ...
            {   //  If the syndromMatrix already exists, make sure that the syndrome matrix
                //  is valid
    ...
    function calculateErrorSyndrome()
    {
        ...
        {
            alert("Recieved code word length must be [length_of_data_sequence + number_of_parity_bits] long.");
            return;
        }

        //  Make sure that the syndrom matrix and parity matrix have been calculated based on the
        //  current data
        ...
        //  convert received data to bianry array for parity bits calculation
        returnValue = stringToBinaryArray(2,
                                receivedData,
                                receivedDataArray,
                                receivedData.length);
